...
PR507 reviewed remote78050104150e2c0c33764f7df0335cbf811507d3 exactly matched all19 local blobs and passed full Linux CI34710017205:4959tests plus37subtests,87.08s. Automatic review comment3997093212 then identified one stale coverage metadata field: events_dropped_by_bound still came from capture summary instead of consumption selection. Root changed this one field to the read-time count. The new regression consumes10fresh publications, then advances the same immutable input beyond24hours:4background events remain and6are dropped by the bound, while the original capture count stays0.192targeted news/compact/release testsPASS in5.63s. User expressly authorized necessary CI/review corrections within PR507 and gated merge/apply; exact-head CI/review must repeat. Server remains a61a22de; no task3 apply yet.

## Active task fx_daily_weekly_context_completion_v1 — perpetual sub-scope

PROJECT=MOEX_Bot. Root rub_snapshot_target_completion_v1, work package rub_analytical_data_gaps_v1. Sole mutation owner fx_implementation; branch codex/fx-daily-weekly-context-completion-v1 from verified clean GitHub main88eff692b59284527e1596b291a924168a991e0d. Root owns publication, independent exact-head review/CI, merge and separate server apply. User authorized GitHub/server access. This entry extends the existing checkpoint; no competing registry.

PM_L1 approved before implementation: reuse trusted accepted Stage7 pointers and existing hash/support/identity/causal checks for USDRUBF/CNYRUBF bounded30D1/up-to8W1 context, exact1/5/20 observed D1 lags, separate dynamic Moscow WTD. Preserve existing accepted W1 and technical blocks; no calendar/session completion inference, historical PIT admission, backfill, trading/model authority or synthetic acceptance timestamp. Actual original build timestamps are retained and additionally gate availability. First acceptance time is unknown where accepted pointer only records acceptance_run_id; never derive it from a run name or read clock. A bad retained row/duplicate or causal hole refuses the window instead of shifting lag targets. Root subsequently authorized sequential CNY H1 and per-contract Si/CR discovery in this same task, pending this first bounded implementation's test freeze.

Root read-only Step0 runtime inventory: each perpetual has1125 acceptedD1 and227W1, observed history2022-04-26..2026-09-11; all12partition/manifest/quality SHA checks passed. Latest accepted W1 is2026-08-31..09-06; observed weekends occur, so weekdays cannot substitute for source dates. Current accepted run step10_daily_20260913_213001, source build2026-09-12T21:34:30Z USD and21:34:47Z CNY. These are accepted partition coverage, not historical vintage/PIT or promised full session coverage.

| Gap | Evidence/classification | Current handling |
| --- | --- | --- |
| Perpetual D1/W1 tails | PROJECTION_GAP; accepted1125/227each | bounded30/8, preserve full partition count/date inventory |
| Perpetual WTD | CALCULATION + PROJECTION | separate observed D1 aggregate, consumption-week bounds and unobserved calendar dates |
| Si FUTOI dated changes | engine/projection tied to current despite Sep11 anchor | separate next scope; existing delta engine retained |
| CR FUTOI | GOVERNANCE_BLOCKED, current-pair-only | unchanged |
| Market OI / basis changes | existing Stage3 OI/Stage4 basis producers; contract runtime coverage unproven | later calculation/projection scope |
| Price causal structure | price-only research formulas RESEARCH_ONLY | no automatic factual promotion |
| Macro previous/decision, US/EIA actuals | calculation/projection or SOURCE_GAP | separate scope; CBR Sep11 official14% unchanged verified by root |
| FRED DEXCHUS history | raw CSV projection gap | CNY, never CNH |
| Contract metadata | currently SECID/BOARDID/LASTTRADEDATE/MINSTEP/STEPPRICE | remaining source/projection gap |
| Minfin | access deferred, mandatory | remains external blocker |
| Requested deeper5m/H1/SiCR history | separate coverage debt | no full historical backfill authorized in this sub-scope |

Implementation: retained immutable OHLCV evidence in native block; compact projection emits one bounded observations list with source provenance, units, explicit unknown first acceptance and observed-return witnesses. One consumption clock recomputes WTD and availability on current/frozen reads. Completeness oracle compares full expected block plus independently checks lag and WTD arithmetic. Local Python compilation and diff check passed; focused Linux tests, real same-input production parity and exact-head review remain pending. No merge/apply or full root completion claimed.

Perpetual first freeze Linux validation by root:162focused testsPASS plus137clock/HTTP/acceptance/package testsPASS. Real current accepted Stage7 pointer reads yielded all four bounded30/8contexts; independent Decimal computation confirms1/5/20returns and observed WTD. This is present accepted input evaluated at control clock, not original full snapshot replay or final production release acceptance. Root retained report fx_runtime_predeploy.json outside repository. Added derived-overflow refusal after the first freeze.

Sequential CNY H1 scope approved by PM_L1: parameterize existing bounded native TradeStats source for explicit USDRUBF/CNYRUBF only, exact endpoint/SECID/units replay, existing12contiguous5m complete-clock-hour rule, finite96h preparation bound. Preserve USDRUBF defaults/live structure/range source; acquire CNY independently and retain each purpose's first acceptance. Both project with source/receipt/original acceptance and no current/PIT/model grant. Root freeze2 Linux219testsPASS for H1,dated,clock,FX,release,HTTP,acceptance,package. Later additive source-units presentation requires final combined rerun. Si/CR immutable archived acceptance-marker route still being assessed; no new archive admission at this checkpoint.

PM_L1 expressly approved BEFORE archived Si/CR admission: CURRENT_REVALIDATED_ACCEPTED_RUN factual scope over existing immutable accepted Stage3 artifacts; absent archived original SHA witness remains historical_original_digest_unavailable. Current revalidation hashes never become original-byte proof or historicalPIT. Require accepted marker identities/references matching existing validated Stage3 specs, original immutable-run declarations, successful Stage10 parent with exact source_refresh run/date, original binding/receipt/source times and parent finish causal at consumption. Exclude standalone marker without parentproof. Preserve original SECID/role binding including expiry, no continuous research-series promotion or backfill.

Si/CR implementation uses cheap date metadata filtering, then at most256recent accepted runs in45calendar days, at most30retained dates perSECID and8descriptive observed-week groups. Old unrelated archive growth does not erase current context; candidate/refusal truncation is explicit. Source5m rows require original identity/date/source, monotonic unique timestamp, per-row receipt<=parentfinish, finite coherent OHLCV. Whole invalid run refuses; sparse gaps stay visible and exact1/5/20trading-date comparisons remain unavailable without complete date universe. Separate contract_price_context survives full/compact/frozen projection and does not upgrade native D1/W1 or readiness.

Root preliminary read-only production evidence: four original contracts SiU6/SiZ6/CRU6/CRZ6 each10accepted observed dates2026-08-28..09-11, including explicit absentSep10; standaloneAug24 refused for missing successfulparent proof. Existing parent records and raw files unchanged. Real CNY H1 independent acquisition/replay passed: Sep11 UTC19:00..20:00,12bars, OHLC12.524/12.532/12.519/12.529, volume37460; newer hour10bars refused, Sep12/13empty. Additional134dated/source/hour/publish Linux tests passed. These predeploy observations and component tests do not establish final production snapshot acceptance; final source/tests now frozen for combined checks, independent exact-head review and CI.

Final initial-task combined Linux validation by root:487testsPASS in29.63s across20affected test files, including full real fixture acceptance-chain collection, receipt/parent rollback/mismatched source negatives,257old-marker growth, CNY/USD H1 firstacceptance and rehashed negatives, all FX bounded tails/lag/WTD cases, compact/current/frozen/reverse oracle and HTTP/clock/package regressions. Local Python compilation and git diff --check PASS. Final exact-head independent review/full CI, publication and separate server acceptance remain root-owned and pending. All source/evidence files remain read-only during collection; no training, trades, signals, Stage5 policy change or claim that root work package is complete.

PR518 automatic-review P2 correction (comment4000393529, threadPRRT_kwDOP-F0yM6h6vum), same task/sole owner: original binding<=parentfinish was insufficient because source ingestion could precede binding on the same day. Require binding availability<=EVERY quote-partition receipt before aggregate admission, preserve the original minimum and maximum receipt timestamps, and recheck binding<=minimum receipt at consumption. A later receipt never excuses an earlier violating row. Added all-early and mixed-receipt negatives, exact-equality positive and read-view refusal. Earlier487affected and5190full Linux passes belong to752914ea and do not validate this correction. Local compilation/diff checks passed; corrected focused Linux results and exact-head review/CI remain pending.

PR518 correction2 after automatic review of de1d18c (threads PRRT_kwDOP-F0yM6h6202/6205/6209), same task/sole owner: validate complete retained binding root/role/instrument/SECID/reference source, source as-of date, nonexpired-at-source expiry, equal original mapping/availability/reference clocks and matching provenance availability. Validate observed date against the45calendar-day Moscow CAPTURE window, not merely row-to-row span. Require canonical portable Stage3 pointer_ref and exact absolute pointer_path when declared, without reopening current pointers during frozen replay.

Adjacent independent audit corrections in the same scope: parent finish cannot follow capture; minimum receipt cannot precede first source bar; bar/gap counts and explicit noncompletion scope remain valid. Retained acceptance/producer run IDs, original run-scoped artifact/marker/parent references, six current-revalidation digests, pass/succeeded/hash semantics are required. Front/next bindings sharing accepted run/date preserve expiry order. No original digest/PIT claim is added.

Completeness oracle independently derives the eligible original SECID/date multiset from source evidence and compares it to output regardless of shared descriptor status. Fault-injection tests replace the shared descriptor to omit one contract, one row, ALL contracts with UNAVAILABLE status, or admit invalid binding; the independent oracle rejects each. Binding-field, stripped-chain, old-single-row/window-boundary, marker-target, capture-clock and row-scope negatives added. Local compile/diff checks passed. Previous490affected/5193full CI results validate de1d18c only; this corrected head requires root Linux tests, independent review and fresh CI/automatic review. No publication or server apply by executor.

Correction2 focused Linux initial run:59PASS,2test-fixture failures. The binding tamper parametrization selected the first collected row (CR), so assigning CR root/instrument did not mutate those fields. Fixture now explicitly selects SiU6 and asserts the replacement differs before mutation; production code unchanged. Corrected test head requires rerun; neither failed fixture case is claimed as a product regression or a passing test.

### 2026-09-14 — Owner handoff: Si dated comparisons, existing PR522

Owner handoff resumes only `si_futoi_dated_comparisons_v1`, branch `task/si-futoi-dated-comparisons-v1`, from exact clean `9af2d9f7d5c2d7e6206694f500097a738f41d970`. PM_L1 confirms PR518 accepted and main/server at `5a63cfaa2d350a942fcb28a0a21723d0057d5971`; previous PR518 pending statements above are historical checkpoint entries. PR522 remains subject to corrected-head tests, independent review, fresh CI and separate production acceptance. Sole executor owns scoped local edits; root owns publishing, merge and server apply.

Review corrections preserve the pre-validation causal cutoff and sample first acceptance only after validation finishes. Cutoff must not precede refresh start or prior accepted/capture/diagnostic clocks. Raw facts are rederived from the same hash-checked, content-address-frozen byte buffer; accepted EOD facts and observed-date witness must match their hash-checked partition buffers, rejecting pointer reread races. Projection validation uses explicit exceptions and remains enforced under Python `-O`. Same-anchor temporary baseline refusal preserves independently valid accepted facts and original first acceptance; latest refusal diagnostics remain separate. A genuine economic revision or new anchor is not concealed.

Si dated output explicitly labels positions/OI in contracts, participants as side counts rather than unique people, net shares and their changes as fractions rather than percentage points, and Si family-aggregate FUTOI rather than individual-contract market OI. CR policy, current live TTL, Stage5 promotion, historical-PIT and trading/model authority remain unchanged. Calendar clock fixture isolates the unrelated Si attachment without disabling its assertions; separate Si tests cover clock order and global rollback.

Added regressions cover mutable source replacement, raw/EOD factual mismatch, observed-date mismatch, optimized-mode rejection, timeline boundaries, transient-baseline retention and output units. Local Python compilation and `git diff --check` pass. Linux focused/full test results for this correction and production acceptance are pending; earlier test totals do not validate this head. No new registry or source backfill was created.

Root focused Linux validation of `a9ae9fbf9c64d3336a92ac65c2aaae63eaba56dc`:116/116 PASS. Independent review then identified final snapshot generation could regress behind completed Si capture while remaining after refresh start. The overlay now rejects generation before `last_capture_attempt_at_utc` before finalization or atomic publication, including failed attempts retaining older accepted evidence. Two integrated clock regressions exercise first acceptance and failed retention. Local compile/diff checks pass; this follow-up requires fresh root validation.

### 2026-09-15 — PR522 runtime accepted; A1 accepted; A2 implementation

PM_L1 confirms PR522 reviewed head `b88f4688d7df682fd221bbb465e913d3ceee2af7`, tree `a4fc54faef1b218503e9e01dbee0c14e1d38c4bd`, merged/applied `bf9bf27f67a75783f0b9a7182b24d3e4652aa586`. Exact CI and isolated server full suite:5613 tests plus37 subtests PASS; independent47 stored-metadata cases and869 output mutations PASS. Two ordinary refreshes retained identical dated evidence `a3af458ce05cbe8f7fa2dbfa7dc02d6d92480c3e17355716078fcb07433bcaf1`, first accepted04:26:59.453479UTC. HTTP/current/frozen/arithmetic/fresh-JSON consumer checks PASS. REAL_ELAPSED expiry04:49:34UTC: saved current became false while dated remained AVAILABLE. No historical reload/deletion; normal per-refresh archive increments14 objects,249358/250540bytes. Root acceptance comment5674904481 records the distinction from simulated negatives. Earlier pending PR522 statements above are historical checkpoint entries.

A1 root data-only acquisition accepted at the same applied bf9bf commit: seven missing Si dates,2828 rows,232079 raw bytes; independently verified actual-buffer hashes, source identity, unique keys, maximum event/sequence selection, integer balance and causal clocks. Existing accepted pointers, Stage5 acceptance maps and protected Aug22/23 bytes unchanged. Actual HTTP05:00:42.827420UTC and CLI05:01:29.664823UTC deliver current20 AVAILABLE from Sep15 to exact Aug24, independently matching frozen anchor and A1 baseline bytes. Invalid latest raw2025-08-11 remains excluded; no Stage5 promotion or whole-history backfill.

A2 `si_futoi_observed_statistics_completion_v1` starts on clean bf9bf under sole local code ownership, root owns publishing/merge/apply. New separately hashed statistics evidence resides inside the existing canonical snapshot and links the unchanged PR522 evidence digest. It stores at most504 observed slots with numeric columns, source clocks and deduplicated proofs. Shared current/dated calculations expose long/short/net/gross, participant side counts and exact1/5/20 changes. Gross share denominator is2*OI; long/short/net use OI. Statistical windows252/504 use the actually admitted subset inside exact slots; missing dates never shift the window. Coverage PARTIAL is separate from statistical availability. PM_L1 explicitly approved NEW descriptive policy minimum2 observations, weak empirical CDF and population standard deviation; constant-sample zscore is null with a reason. This is neither the old full-N warmup nor confidence/model authority.

Source capture checks frozen raw byte-to-fact agreement, mandatory raw publication/ingest clocks and accepted EOD bytes. Existing invalid raw excludes its date without older-revision/EOD fallback. Consumer current statistics replace legacy EOD-plus-current statistics or emit an explicit refusal. Compact output contains summary metrics, exact sample dates/exclusions and a locatable audit reference, not504 full factual objects. Independent Decimal arithmetic plus explicit output metadata, counts, exclusion inventories and refusal checks cover shared-render faults. Source freshness, CR policy, training/trading prohibitions and overall PARTIAL/Minfin blocker remain unchanged. Compilation passes; A2 focused/full tests, independent review, CI and real production acceptance remain pending.

### 2026-09-15 — A2 merged/applied; bounded consumer-semantics correction

Root confirms PR523 merged/applied `180e39e8156b1ddd27c94b75b560793b18627f30`. Real source audit verified502 historical facts plus current against source bytes;440 independent Decimal checks PASS. Actual reported coverage is251/252 and502/504, not the earlier anticipated counts. Invalid latest raw2025-08-11 and2026-03-20 remain invalid in bounded fresh official responses; no provider correction is claimed. Final A2 runtime acceptance remains pending consumer clarity correction and remaining retention checks.

Task `si_statistics_consumer_semantics_v1` is a separate output-only correction from clean merged180e39. JSON now states weak empirical CDF `count(sample <= anchor)/N`, inclusion of all ties and the anchor, population z-score with ddof0, minimum2 and explicit zero-variance null reason. Only the mixed statistics block drops the ambiguous inherited top-level current_usable flag; the original Si dated flags are unchanged. Current and dated views explicitly identify anchor role, current-pair usability at read, and original source anchor clocks. Current usability still requires existing current admission; no authority is added. Canonical refusal and independent metadata verification apply to both primary views and the legacy current alias. Stored evidence/schema, hashes, linked identity, capture logic, first acceptance and arithmetic are unchanged. Focused/full tests, independent review, CI and corrected runtime acceptance are pending; no A3 implementation has started.

### 2026-09-15 — PR524 merged/applied; A3 scoped CR implementation

PM_L1 confirms PR524 merged/applied `1d9b1d33b77d36f72013459012d3c8231772882e` and releases A3 `cr_futoi_dated_comparisons_v1`. The earlier pending/no-A3 statements above record their historical checkpoint time. A3 starts from this exact clean base on `codex/cr-futoi-dated-comparisons-v1`; root owns publication, merge, application and independent runtime acceptance.

Root data-only CR acquisition completed at2026-09-15T06:36:26.729158UTC:12 missing Aug18–29 dates,4288 rows,376985 bytes. Independent actual-buffer audit PASS for hash/manifest/quality, identity, latest event/sequence/shared session, integer net/OI and causal clocks. All accepted pointers, Stage5 maps and five protected historical CR buffers remain unchanged. This supplies exact Aug23 dated20 and Aug24 current20 targets; it does not retroactively establish historical point-in-time availability. Some earlier intraday pairs are invalid although the latest pair passes; no whole-partition or session-completion claim is made.

A3 adds a separate CR admission artifact, retained snapshot store and scoped consumer subtree. It retains at most21 exact observed dates, including explicit unavailable placeholders, and exposes dated previous-anchor plus current exact1/5/20 long/short/net/gross, shares and participant-side changes. Gross share uses2*OI. Existing legacy CR previous/comparison fields and global current-pair-only governance remain unchanged; Si evidence and first acceptance are untouched. Dated acceptance/source age is bounded96h; current additionally requires the existing current admission and original source TTL. Reads recheck the current scoped grant. Capture verifies frozen raw bytes against the selected factual pair, requires original raw publication/ingest clocks, and uses accepted EOD only when exact raw is absent. Invalid latest raw never falls back to an older pair or EOD. Current retains its own frozen raw and publication-audit witness. Repeated equivalent observations preserve first acceptance; failed retries retain independently valid facts with visible diagnostics. No statistics or model/trading authority is added; CR statistics remains A4.

New tests cover exact current/dated ordinal targets, explicit missing baselines, current refusal while dated survives, grant revocation, raw clock/identity/digest failures, EOD clock semantics, retry retention, clock reversal, nested metadata filtering and independent arithmetic/output tampering including optimized Python. Local compilation and whitespace checks are required before freeze. Focused/full Linux tests, independent review, CI and A3 real runtime gates remain pending; the overall execution stays PARTIAL with the existing Minfin blocker.

PR525 initial head `0235d49ab4bcfd8dc0d97d7d7ebbd3794a315a12` passed508 focused tests, full5737 tests plus37 subtests, CI34939449374 and independent2043 optimized-Python mutations plus168 Decimal checks. Automatic review then identified two material gaps, so these passes do not establish completion. The same-task correction limits retention to explicitly classified transient I/O failures; a newly invalid latest baseline stays unavailable, and a newly invalid latest anchor blocks admission through timestamped rejection metadata while preserving the original evidence/first-acceptance bytes. A real-parquet regression appends an invalid newer pair after a valid pair for both cases. Complete scoped contract identity, inventory and JSON value types are now bound, including task_id, latest-source selection and earlier-invalid-pair semantics. Corrected focused/full/review/CI and real runtime gates remain pending; no merge or application is claimed.

Correction head `d22fef1c700aec1a628220320cda8516a2bfc1eb` passed full5779 tests plus37 subtests and CI. Independent lifecycle review then found that repeated invalid anchors advanced the effective rejection time, allowing an earlier as-of replay to resurrect old availability. The follow-up preserves the first rejection clock/date/reason until valid source re-admission, and records later attempt times and changed errors separately. Tests cover repeated identical rejection, changed reason, invalid-to-transient retry and valid recovery. These new changes require fresh exact-head gates; A3 remains unmerged and unapplied.

Head `c202fb5e90594c5c13f575a7e52eaf9a78dd3b96` passed full5782 tests plus37 subtests and CI. Automatic review identified a diagnostic-only gap: a later transient anchor read failure retained the first rejection correctly but replaced its actual read error with a generic admission failure. The minimal follow-up preserves the first semantic rejection and records the exact latest unavailable-anchor reason, including transient I/O errors. Lifecycle tests now assert the complete latest error for every retry case. New exact-head gates remain required; admission behavior is unchanged.

The bounded diagnostic audit of `0b3b1e18b44108201b24685592c5c1956c4a74ae` additionally found lost causes on first dated capture failure and additional current frozen-byte capture failure. The combined correction stores timestamped nullable-text operational diagnostics outside accepted evidence and exposes validated diagnostics in both admitted and refused CR scoped JSON. This preserves the first effective rejection while making the latest failure separately visible. Future diagnostics are hidden at earlier reads; malformed metadata/read clocks produce canonical refusal without leaking objects. No accepted store is fabricated for first failure, no missing current byte proof is bypassed, and original accepted evidence/first acceptance remain untouched. Tests cover both causes, invalid/future clocks, malformed diagnostic inventory/types and refused-output tampering. All corrected exact-head gates remain pending.

Independent follow-up on `614bfac95be1d9dd95b7eff205a759da7bb2117e` found that well-shaped diagnostic metadata could contradict its capture evidence. The next correction binds diagnostic time/error to the stored attempt/error, requires current proof time agreement and absence of current error, and rejects claimed successful capture without its store/proof. Finalized refresh boundaries constrain diagnostic time where those boundaries exist. Prior first-failure diagnostics also constrain the next capture clock, even without an accepted store. Coherent future diagnostics remain hidden at earlier reads; absent diagnostics retain compatibility. New contradiction, clock-reversal and future-read regressions require fresh exact-head gates. A3 remains unmerged and unapplied.

Head `4bd42839476864ccd1f33dc7a67c36d7e24ac386` passed full5804 tests plus37 subtests and CI, but automatic review found that a claimed successful current capture could carry malformed proof bytes/digest. The follow-up validates current capture carrier/hash, static factual clocks/identity/arithmetic and publication-audit/raw-proof linkage before exposing success diagnostics. The same static helper serves the current reader without recursing through diagnostics. Successful dated capture is validated against its retained admission at the recorded attempt time. Later current TTL expiry, dated expiry or present grant revocation remain read-time conditions, not retroactive capture failures. Malformed current/dated carrier, digest, factual and audit regressions plus later-expiry positives are added; fresh exact-head gates remain pending.

Independent review of `cee7f98bff6b7a1ebf6b9c6675b91b22080a4a7d` identified a remaining capture-time constraint: internally consistent source clocks could already be expired at the stored causal cutoff. The correction reuses the existing current-pair MAX_AGE_SECONDS=1200 for all four factual clocks against that cutoff. It does not apply the capture freshness bound to later completion or consumption time. Exact-boundary, just-expired, one-day-shift and later-completion regressions supplement the existing legitimate later-expiry test. Local compile/diff checks and fresh exact-head focused/full/review gates are required; no merge or application is claimed.

Automatic review of `f53d730e3aada183eeb89b9967256871fff88f75` identified missing original source-attempt predicates. PM_L1 authorized retaining the complete bounded original admission inputs: status, failed_attempt_at, original last_success_at, original governance bytes/hash, and referenced legacy acceptance evidence bytes/ref/hash. Static capture validation now calls the existing full check_time at the original cutoff and mirrors original accepted/scope/required-gate/acceptance-evidence predicates. Publication-audit factual/source-byte linkage and required admission-result fields remain checked. Capture revalidates the retained carrier after reading the same referenced evidence bytes; current reads bind the source envelope to the original record and retain their existing current gate. This adds no grant, signing system, registry or Si change. Missing/stale/failed source attempts, original governance/evidence mutations and current-envelope mismatch have targeted regressions. Original source receipt, local completion and later read times remain distinct; fresh exact-head gates are pending.

Automatic review of `003379abf1324d3a8d359fe0ebcfab9c547029f0` found EOD read failures were collapsed into permanent absence before per-date source selection. The bounded correction preserves the EOD loader/ref-verification/frame-read exception and consults it only when the exact raw partition is absent. Typed I/O failures therefore retain their transient classification, while invalid EOD and true absence remain refusals. Valid raw does not depend on EOD availability; invalid latest raw never falls back. Regressions cover raw valid/invalid/missing, valid/absent/invalid/unreadable EOD, each of the three EOD failure stages for anchors/baselines, and witness read failure retaining original evidence with its cause. Independent review confirmed the original defect and bounded selection behavior; fresh exact-head gates remain pending.

Head `abb30700fe44aecfaf7721ea4f2a9321e97846cb` required a fixture-only data-root isolation correction (`ab15fb7e8b00ae074c735e588eb3217da2712582`), preserving every assertion. Its completed automatic review also identified that real Step9 readers wrap I/O failures in Step9AnalysisBundleError. The combined correction follows explicit causes through this known transport wrapper with cycle protection; it does not follow incidental exception context or arbitrary validation wrappers. Actual _accepted_eod→_read_pointer_block→_load_json read faults now preserve transient classification, while malformed JSON/schema remains definitive. Tests exercise the real wrapper path for anchor/baseline retention, JSON syntax/schema negatives and causal-cycle handling. Source precedence, original admission proof and authority policy are unchanged; new exact-head gates remain pending.


A3 PR525 is merged/applied at `8dce83d3097cc535d3787a7cc70e2838ccd16519`. Actual CR dated source bytes, 84 independent Decimal checks, HTTP/CLI and frozen replay passed; dated evidence `65f36…` retains first acceptance 09:40:41 UTC. The current runtime gate remains open: a valid 10:00 generation was followed by a latest-source FIZ/YUR imbalance at 10:10, and the actual 10:13 consumer correctly refused current data. This is not full current-runtime completion.

A4 `cr_futoi_observed_statistics_v1` adds a separate scoped CR statistics grant/store and compact dated/current summaries over exact witnessed 252/504 slots. Missing or invalid slots remain excluded rather than replaced by older observations; available subsets use an explicit minimum-two descriptive policy, weak-CDF ties, population variance and a null z-score for constant samples. Four historical invalid raw dates (2025-06-30, 2025-08-11, 2026-01-16, 2026-03-20) were independently confirmed against fresh official responses; candidate counts 250/252 and 500/504 require actual A4 runtime measurement before acceptance. No historical acquisition, Stage5 promotion, legacy CR statistics alias or authority extension is included.

The shared kernel contains only neutral row codecs, descriptive arithmetic and an independent per-view verifier. Nine Si capture/admission/semantic/retention/read functions remain AST-identical to the applied base; golden serialized input/output tests cover 1/30/504 slots. Real Si publication-clock revisions retain their original source-version semantics. CR uses its own 96-hour first acceptance linked to A3 evidence, original current admission proof, immutable source references, transient-read diagnostics and first effective anchor rejection. Local compilation and diff checks are preparatory only: focused/full Linux tests, golden execution, independent review, CI and actual A4 runtime acceptance remain pending.

A4 initial head `9326a160c7a6f3d9041d35ae2e0d23a963da60d4` passed 703 focused Linux tests. Independent review identified that exclusion diagnostic wording participated in semantic identity. The correction preserves original accepted evidence/hash/time when only wording changes, while exposing the new message through validated operational sample diagnostics. Status, original source clocks and excluded raw source hashes still distinguish real source revisions. Targeted message-only versus source-hash-change regressions require fresh exact-head verification.

A4 head `048eb2be7c710dd8eb44b64b8f58bfb85e914438` passed 705 focused tests and full 5934 tests plus 37 subtests on Linux/CI. Automatic review identified an atomic raw-file replacement gap: an initial invalid read could be paired with a hash of later valid bytes. The correction revalidates the exact frozen buffer with the canonical latest-pair parser before assigning an exclusion proof; valid replacement bytes supply their own facts and original clocks. Tests replace the canonical inode between its first read and hashing in both directions. New exact-head gates remain pending.

Updated A3 runtime evidence supersedes the earlier transient source-blocked status: actual 10:20 and 10:31 current/dated source-proof runs each passed 168 Decimal checks and HTTP/current/frozen parity; immutable CR evidence 65f36… and first acceptance 09:40:41 remained stable. Real elapsed expiry at 10:35:20 correctly refused current while retaining dated availability. Fresh JSON-only transfer approval and separate Si changed-source comparison remain pending; historical intermittent +430 source refusal was not bypassed. A4 coverage counts remain unmeasured candidates until its own runtime acceptance.


A4 PR527 is merged/applied at `d6ffd66f6bcc0bd30e2d0997db6686033af5dc31`; Linux/CI passed 5936 tests plus 37 subtests, independent review and automatic review completed, and server runtime checks are complete. The separate fresh JSON-only consumer gate remains pending authorization. Detailed runtime evidence remains in local acceptance records.

B `contract_price_market_oi_observed_comparisons_v1` starts from that applied base. The initial implementation extracts the existing accepted Stage3 resolver without weakening the legacy parent gate, adds a distinct bounded standalone Aug24 admission and exact-byte paginated Aug23 parser, and pairs quote/OI within the same accepted run, SECID, date and common timestamp. Baseline roles refer to the anchor binding, not an invented historical role. New current comparisons require original native HTTP entity bytes from the existing refresh, with individual receipts, completeness evidence and same-row LAST/OPENPOSITION; no additional network requests are introduced. Native row-update time is explicitly distinguished from last-trade time. Market OI stays source-native as reported, without rescaling or substitution for family FUTOI totals. Existing contract_price_context and 22 live basis formulas remain unchanged.

B is an initial review/test freeze, not completion: source/admission/retention/oracle tests and simulated consumer artifacts are still being extended. Local compilation and a temporary-directory rehearsal of all ten already acquired public Aug23 response/receipt buffers passed, reproducing the four exact 19:00 MSK price/OI pairs; the rehearsal used a synthetic validation clock and is not production admission. No B source installation, deployment or runtime acceptance is claimed. Exact-head Linux/full/CI/independent review and actual source installation/runtime gates remain required.


### B source-byte and fast-current correction — 2026-09-15, verification pending

Initial B head `bf20a5ddcc00dbab5ce6509ff06fd50fa9c8db65` passed 854 focused Linux tests. Independent review nevertheless reproduced a material portable-proof omission: modified copied rows could disagree with the original source bytes while remaining admitted. That initial result is not a completion gate.

The correction retains bounded, SHA-deduplicated original buffers in the full audit evidence and rederives Stage3 paired records, all ten pilot/support identities, the accepted Stage7 observed-date witness, the bounded official acquisition, and native security-derived role bindings from those buffers. Final standalone marker/pilot hashes must match their separate admission. Compact output retains summaries and the audit reference; selection-role time is explicitly separate from historical bar dates. Native buffers use inline content references, not invented server file paths.

Fast-market refresh now carries its own original-byte current proof in the existing overwritten fast state; read overlay changes only current evidence, with no network call or dated reacceptance. The existing 2 MB fast-state bound remains enforced, and CLI collection diagnostics exclude source buffers. Completion is sampled after byte validation. Limits for the full dated audit are 1024 unique buffers, 8 MB per buffer and 64 MB decoded total, with fail-closed overflow; actual production inventory and serialized-size verification remain pending. No production admission, apply, backfill or data deletion has been performed by this implementation task.

Local compilation and whitespace checks pass. Corrected focused/full Linux, independent source mutation checks, CI, synthetic JSON-only consumer review, and runtime size/HTTP/CLI acceptance remain pending on the corrected exact head.


B follow-up: intermediate `052c0415f9d3aad4b8948425da1cc0863a81c042` focused Linux completed with 899 PASS / 2 fixture failures (zero-OI copied-row fixture and mutation of a shared probe list). Both fixtures now use the correct original-buffer semantics. Actual source inventory also exposed a collector-only schema mistake: real pilot documents have `artifact_version`, not top-level `run_id`; run selection now derives identity from the canonical marker path and preserves the existing later admission validation. The regression traverses actual filesystem archives and verifies that many recent runs on one date do not displace other observed dates. Cross-platform retained paths use lexical POSIX/Windows interpretation without filesystem reads during replay.

The synthetic consumer fixture now admits exact Aug23 official-page and Aug24 standalone partial endpoints, with current/dated 1/5/20 targets and separate refusal examples. These are explicitly generated simulation buffers, not production evidence. Corrected final focused/full/CI, measured full archive sizes, and synthetic consumer review remain pending.


PR528 correction: the previous exact head passed full Linux/CI (6002 tests plus 37 subtests), but automatic review identified that the run budget was checked only after collapsing runs into dates. The corrected collector limits directory enumeration before opening pilot or parent content: all directory entries count, including entries outside the later 45-day date filter. Entry 257 causes an explicit inventory-limit refusal; nothing is truncated and no older run is substituted. Within the admitted inventory the latest-per-date selection is preserved. Pilot/parent content reads also use the bounded source reader, and metadata byte overflow is an explicit refusal. Existing accepted evidence may remain readable only under its original first acceptance and expiry, with the new collection error visible. No archive deletion or new index is introduced. Boundary, zero-content-read, retry-selection and retention regressions accompany the change; new exact-head gates remain pending.

The same correction adds a comparison-only continuity guard: a fresh current source date more than one calendar day beyond the retained observed witness keeps its verified anchor and distribution, but all exact current 1/5/20 targets and numerical changes are explicitly unavailable. The rule makes no weekend or non-session inference. Dated evidence and first acceptance remain unchanged. Original-response regressions cover same-day, next-day, a missing intervening date, and a weekend gap; the independent oracle checks refusal metadata and compact parity.

PR528 additional bounded-read correction: the prior exact head passed CI (6012 tests plus 37 subtests) and independent review, but automatic review found a first-read bypass in accepted-marker resolution. The correction adds an optional byte reader through the resolver and existing Stage3 support/causal validator, leaving the default legacy path unchanged. B reads original artifacts with its existing per-file and total limits, memoizes each run's bytes, and freezes those same validated bytes. The witness now validates bounded pointer/support/partition buffers with its existing portable admission guards; existing frozen-copy comparison is also bounded. Regressions cover marker, quote/OI/TOM support, OI partition, witness pointer/support/partition, unchanged legacy specs, causal/governance refusal, and replacement after validation. Compilation and whitespace checks pass; all new exact-head Linux/CI/review gates remain pending.

PR528 parser/inventory follow-up: the previous exact head passed full Linux/CI (6032 tests plus 37 subtests), but automatic review found lost strict JSON hooks and decoding before complete inventory limits. The bounded resolver and B source/portable readers now retain Step9 duplicate-member/nonfinite rejection, including witness and original marker/pilot/parent documents. A strict prefilter failure is explicit rather than silently selecting an older candidate. Native-response and retained-buffer inventories are fully size-checked before the first decode; native budget counts repeated response entries, while identical encoded buffers are decoded once. Conflicting bytes under a repeated claimed digest refuse. The existing 8 MB per-buffer and 64 MB aggregate limits remain; portable response parsing has the same aggregate check. New parser/parity, self-hashed replay, no-decode-on-overflow, exact-boundary and duplicate-buffer regressions accompany the correction. New exact-head gates remain pending.

The parser/inventory head's CI run exposed eight fixture failures (6062 tests plus 37 subtests passed). Retention fixtures now supply the existing original-response carrier so the intended candidate reaches admission while the injected current-read failure remains. Malformed witness support uses a distinct frozen file/reference, avoiding unintended invalidation of another support document that shared the original digest. All substantive retention and strict-parser assertions remain; product code is unchanged by this fixture correction. New exact-head verification is pending.

PR528 subsequent review correction: parent attempts are ranked per source date before their status/rollback/identity admission, so an invalid newer parent cannot silently expose an older accepted run. A parent whose ordering cannot be established explicitly refuses that date. Capture assigns an admissible candidate only after both dated and current original-buffer construction succeed; failures retain their original diagnostic instead of becoming an incomplete-evidence shape error.

The review's claim that the existing no-cursor source verifier compares full live rows was checked against its implementation: it returns and compares normalized SECID sequences. B now reuses that helper, including within-response securities/marketdata universe equality, and rejects a cursor appearing only in the probe. Cursor pagination also preserves source column-schema and request-start progression gates. Different LAST/OI/expiry values between the two source requests remain permitted under the existing source policy; no full-row equality policy is introduced. Negative acquisition/replay tests and positive changing-value tests accompany the correction. New exact-head Linux/CI/review gates remain pending.

The cursor parity review additionally identified case aliases in the total-cardinality check. B now counts SECIDs with the original source merge's `str(...).upper()` normalization. The negative fixture retains all four required contracts and adds a differently cased alias across pages; the original source merge produces fewer unique contracts than declared TOTAL, and B rejects the same inventory. Local compilation/whitespace checks pass; server tests remain pending.

PR528 future-parent correction: a known source-date parent finishing after the capture cutoff now explicitly refuses that date with the original causal rejection reason. It cannot be omitted to expose an older accepted run, and no future evidence is admitted. The paired older-valid/newer-future regression also confirms other dates remain readable. Local compilation and whitespace checks pass; new exact-head gates remain pending.

The source completeness helper's typed runtime error is now adapted to B's ValueError refusal boundary with its original message and exception cause preserved. This prevents malformed source universes from escaping portable describe/oracle handling. Acquisition negatives remain strict, and a self-hashed portable universe-mismatch case verifies current refusal with dated evidence still available. Prior cursor-head CI completed with two exception-contract failures; the combined corrected head requires renewed verification.


PR528 capture-resource correction: the previous head passed full Linux and CI (6101 tests plus 37 subtests), but automatic review found that per-run limits allowed cumulative freezing before the final audit limit. One explicit capture budget now covers discovery metadata, the observed witness, normal and alternate historical admission, native responses and final buffer construction. It permits at most 1024 distinct original buffers totaling 64 MB; identical bytes across paths are charged once, while different versions count separately. The existing 8 MB individual-buffer limit and repeated-native-response inventory limit remain independent. An unknown digest requires one bounded read/hash before charging; this does not claim a 64 MB bound on all repeated reads or peak process memory. Overflow is sticky and refuses the whole capture before further parsing or freezing, preserving prior evidence and first acceptance with the precise resource diagnostic. Freeze reserves the budget before directory creation or writing. Boundary, shared-source, alternate-route, no-read/decode/write-after-overflow and retention regressions accompany the change. Local compilation and whitespace checks pass; renewed exact-head Linux, CI and review gates remain pending.


PR528 fast-state size correction: the prior exact head passed full Linux and CI (6111 tests plus 37 subtests), but automatic review found that optional current proof overflow discarded independent dated market evidence. Fast persistence now removes the optional B proof first, retaining ordinary market and dated evidence when they fit. If necessary it refuses ordinary market while keeping dated evidence, then revalidates the previous dated witnesses without new observations or acceptance clocks; a final bounded refusal handles oversized dated state. Every persisted result remains within the existing UTF-8 2 MB cap. Fast overlay explicitly clears missing B current proof, so no slow-current fallback is introduced. The byte-limit diagnostic remains visible. Regression coverage includes persistence, overlay, subsequent failed refresh, independent dated expiry, original frame/hash preservation and multibyte size limits. New exact-head verification is pending.


B runtime parity correction: PR528 is merged and applied at `bedf67672d0152f340c79e4c94a7a9f643cb603d`; full B runtime acceptance remains pending. Runtime replay exposed a representation mismatch: the original reader normalizes native LAST to float, while portable reconstruction retained JSON integers for whole-number prices. The corrective task reuses the original price/integer helpers for canonical native facts and reconstructed reader nodes, retaining original response bytes and copied proof rows unchanged. Strict source-value, clock and digest comparisons remain enforced. Regression cases cover integer Si prices, fractional CR prices, integral OI representations, invalid fractional OI, byte preservation and tampered-value refusal. Local UTF-8/raw-byte compilation and whitespace checks pass; new exact-head Linux, CI and independent runtime gates remain pending.


B consumer-semantics correction follows applied `872c2267cf0eaea2c53982d1a7cdba7910616bfa`. JSON-only review identified missing distinctions between native LAST and historical five-minute CLOSE, source endpoint/publication/receipt clocks, exact observed-date horizons, dated first acceptance and current freshness. Presentation now explains these distinctions, the 96-hour dated deadline, missing-target/current-witness policy, selected-role limitations and current-pair usability. Compact output retains the existing dated evidence SHA beside its audit reference without embedding source buffers. The original admission contract is intentionally unchanged because its exact text is part of retained evidence; producer, admission, stored digest, first acceptance and source values are unchanged. A frozen pre-change renderer verifies all prior output fields, with independent metadata-tamper and compact traceability regressions. Local raw-byte UTF-8 compilation and whitespace checks pass; exact-head tests, independent review and runtime consumer acceptance remain pending.


C1 historical basis/carry implementation follows final B acceptance (PR530, applied `fa1f4efaf26d588f18ab2a7efbef837337ce3fa0`). The separate dated contract/store revalidates original Stage4 pilot, accepted marker, successful parent, derived partitions and all eight own source legs from bounded same-byte buffers. Portable replay repeats newest-run/date selection and own-leg arithmetic. Per-metric exact timestamp intersections preserve futures-only endpoints while USD spot-dependent metrics remain refused. Exact witnessed 1/5 targets are distinct from the nearest previous comparable observation; no sparse-success indexing, fill, rollover splice, session completion or historical PIT claim is introduced. Dated first acceptance is immutable for identical original evidence, including after its 96-hour lifetime; ordinary refresh cannot rejuvenate expired facts. A genuinely changed original source version receives a new acceptance only after full validation. Current C2 admission remains explicitly unavailable and requires its separate future contract/proof. Existing live formulas, CETS acquisition and fast transport are unchanged. The optional Stage4 byte reader preserves legacy default behavior; existing finite calendar-clock assertions remain intact while unrelated C capture is isolated. Initial C1 local compilation/whitespace checks pass; exact-head focused/full Linux, CI, independent source/Decimal review and runtime acceptance are pending. Overall C remains incomplete until C2.

C1 initial focused verification: 41 tests passed; two fixture expectations used an unintended weekend-inclusive witness. The corrected fixture declares its observed dates explicitly and separately verifies weekend observations are retained when supplied. The same correction closes expired-source reacceptance with unchanged-source and genuine-version-recovery regressions. Renewed exact-head tests remain pending.

C1 bounded source reader checks Parquet footer row/column counts before either the shared physical validator or portable replay materializes the same verified bytes. Exact 5000-row/100-column boundaries and pre-decode overflow refusals are covered. This is not a peak-memory guarantee from compressed-byte size; legacy callers without the optional reader retain their existing behavior. Exact-head verification is pending.

C1 consumer metadata explicitly defines normalized own-close rates, comparison/reference orientation, basis points and simple annualization using 365 calendar days, including spot and term tenor definitions. Clock metadata separates bar endpoints, unknown price publication, original receipts, binding observation and first C1 acceptance. Only presentation and its independent oracle change; the admission contract, source custody, stored evidence and first acceptance remain unchanged. Formula/unit and metadata-mutation regressions accompany this clarification; final exact-head verification is pending.

C1 integration verification found an existing hourly-source fixture whose finite clock sequence did not isolate the new unrelated capture. Only that fixture now isolates C1; all original network receipt and final hour acceptance assertions remain unchanged. A separate integration regression runs real C1 capture timing, verifies canonical generation follows completion, and prevents publication when the final clock regresses. Production code is unchanged; renewed exact-head tests are pending.

The completed superseded CI run also identified the same unrelated-clock consumption in both variants of the existing Si generation-regression fixture. That single fixture now isolates C1 while retaining all Si completion, failed-attempt retention and no-publication assertions. Production code remains unchanged; final exact-head focused/full/CI verification is pending.
