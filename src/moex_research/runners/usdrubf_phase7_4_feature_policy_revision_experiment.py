...
from .usdrubf_phase7_4_feature_policy_revision_builder import IDENTITY_COLUMNS, MATRIX_CATEGORICAL_FEATURES, MATRIX_NUMERIC_FEATURES, M1_CATEGORICAL_FEATURES, M1_NUMERIC_FEATURES, NEW_NUMERIC_FEATURES, PHASE6_IDENTITY_AND_TARGET_COLUMNS, PHASE6_LEGACY_CATEGORICAL_COLUMNS, PHASE6_LEGACY_NUMERIC_COLUMNS, SOURCE_REQUIRED_COLUMNS, FeatureBuildResult, build_feature_matrices
...
DESIGN_RECOMMENDATION_ID: Final[str] = 'usdrubf_phase7_4_feature_policy_revision_design_v4_complete_corrected'
PROJECT: Final[str] = 'MOEX Bot'
LANE: Final[str] = 'ema_3_19_ai'
TASK_ID: Final[str] = 'ema_3_19_ai_market_phase_phase_7_4_feature_policy_revision_implementation'
...
PROBABILITY_COLUMNS: Final[tuple[str, ...]] = ('probability_B', 'probability_S', 'probability_OUT')
M0_REQUIRED_COLUMNS: Final[tuple[str, ...]] = ('fold_id', 'target_trade_date', 'target_instrument_id', 'y_true', 'candidate_y_pred', *PROBABILITY_COLUMNS)
REQUIRED_CLI_ARGS: Final[tuple[str, ...]] = ('--source-panel-path', '--source-panel-manifest-path', '--modeling-dataset-path', '--dataset-manifest-path', '--feature-schema-path', '--readiness-contract-path', '--m0-validation-predictions-path', '--experiment-contract-path', '--output-dir', '--run-id', '--git-commit-sha')
SPLITTER_CONSTRUCTOR: Final[dict[str, int]] = {'n_splits': 5, 'test_size': 64, 'gap': 0}
MODEL_CONSTRUCTOR: Final[dict[str, Any]] = {'C': 1.0, 'class_weight': 'balanced', 'solver': 'lbfgs', 'max_iter': 1000}
DECLARED_OUTPUT_ARTIFACTS: Final[tuple[str, ...]] = ('input_identity_verification.json', 'feature_matrix_inventory.json', 'feature_nullness_by_matrix_and_fold.csv', 'feature_shift_by_matrix_group_and_fold.csv', 'fold_metrics_by_matrix.csv', 'aggregate_metrics_by_matrix.json', 'per_class_metrics_by_matrix.csv', 'ablation_effects.csv', 'gate_results.json')
MATRIX_ROLES: Final[dict[str, str]] = {'M0_FROZEN_PHASE7_2_CONTROL': 'immutable_historical_control', 'M1_REVISED_FULL': 'sole_acceptance_candidate', 'M2_MINUS_NORMALIZED_EMA_TREND': 'diagnostic_only', 'M3_MINUS_VOLATILITY_RANGE': 'diagnostic_only', 'M4_MINUS_VOLUME_ACTIVITY': 'diagnostic_only', 'M5_MINUS_LAGGED_INTERSESSION_GAP': 'diagnostic_only'}
REVISED_GROUPS: Final[dict[str, tuple[str, ...]]] = {'lagged_intersession_gap': ('lag1_intersession_gap_days',), 'normalized_ema_trend': ('lag1_ema_3_19_spread_pct', 'lag1_close_ema_19_distance_pct', 'lag1_ema_3_slope_5_pct', 'lag1_ema_19_slope_5_pct', 'lag1_ema_3_19_spread_pct_change_5', 'lag1_ema_3_19_state_run_length_log'), 'volatility_range': ('rolling_past_return_std_5', 'rolling_past_return_std_20', 'rolling_return_std_ratio_5_20', 'lag1_hl_range_pct', 'lag1_hl_range_to_prior20_mean', 'rolling_hl_range_mean_ratio_5_20'), 'volume_activity': ('lag1_log_volume_rel_prior20', 'lag1_log_num_trades_rel_prior20', 'lag1_log_avg_trade_value_rel_prior20', 'rolling_log_volume_mean_diff_5_20')}
M0_GROUPS: Final[dict[str, tuple[str, ...]]] = {'lagged_intersession_gap': ('session_index', 'days_since_prior_trade_date'), 'normalized_ema_trend': ('lag1_ema_3', 'lag1_ema_19', 'lag1_ema_3_19_spread'), 'volatility_range': ('rolling_past_return_std', 'lag1_hl_range_pct', 'rolling_past_hl_range_mean', 'rolling_past_hl_range_std'), 'volume_activity': ('lag1_volume', 'lag1_value', 'lag1_num_trades', 'rolling_past_volume_mean')}
_ALIAS_PATTERN = re.compile('(^|[/\\._-])(latest|current|autodetect)($|[/\\._-])', re.I)
_SHA_PATTERN = re.compile('^[0-9a-fA-F]{40}$')
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog='python -m moex_research.runners.usdrubf_phase7_4_feature_policy_revision_experiment')
...
def request_from_args(args: argparse.Namespace) -> Phase74ExperimentRequest:
    request = Phase74ExperimentRequest(source_panel_path=_explicit_path(args.source_panel_path, '--source-panel-path', ('.parquet',)), source_panel_manifest_path=_explicit_path(args.source_panel_manifest_path, '--source-panel-manifest-path', ('.json',)), modeling_dataset_path=_explicit_path(args.modeling_dataset_path, '--modeling-dataset-path', ('.parquet',)), dataset_manifest_path=_explicit_path(args.dataset_manifest_path, '--dataset-manifest-path', ('.json',)), feature_schema_path=_explicit_path(args.feature_schema_path, '--feature-schema-path', ('.json',)), readiness_contract_path=_explicit_path(args.readiness_contract_path, '--readiness-contract-path', ('.yaml', '.yml')), m0_validation_predictions_path=_explicit_path(args.m0_validation_predictions_path, '--m0-validation-predictions-path', ('.parquet',)), experiment_contract_path=_explicit_path(args.experiment_contract_path, '--experiment-contract-path', ('.json',)), output_dir=_output_path(args.output_dir), run_id=str(args.run_id).strip(), git_commit_sha=str(args.git_commit_sha).strip().lower())
...
def _input_paths(request: Phase74ExperimentRequest) -> dict[str, Path]:
    return {'source_panel': request.source_panel_path, 'source_panel_manifest': request.source_panel_manifest_path, 'modeling_dataset': request.modeling_dataset_path, 'dataset_manifest': request.dataset_manifest_path, 'feature_schema': request.feature_schema_path, 'readiness_contract': request.readiness_contract_path, 'm0_validation_predictions': request.m0_validation_predictions_path, 'experiment_contract': request.experiment_contract_path}

def _prepare_experiment_eligible_rows(dataset: pd.DataFrame) -> pd.DataFrame:
    required = (*PHASE6_IDENTITY_AND_TARGET_COLUMNS, *PHASE6_LEGACY_NUMERIC_COLUMNS, *PHASE6_LEGACY_CATEGORICAL_COLUMNS)
    ...
    eligible = rows['target_source'].eq(TARGET_SOURCE) & rows['target_is_labeled'].eq(True) & rows['target_phase_label'].isin(CLASS_ORDER) & dates.notna() & instruments.notna() & instruments.ne('')
...
def _expected_validation_identities(eligible: pd.DataFrame, folds: list[tuple[np.ndarray, np.ndarray]]) -> pd.DataFrame:
    frames = []
...
def validate_m0_predictions(predictions: pd.DataFrame, *, expected_fold_identities: pd.DataFrame) -> pd.DataFrame:
    ...
    parsed = pd.to_datetime(observed['target_trade_date'], errors='coerce')
    if parsed.isna().any():
        raise Phase74FeaturePolicyExperimentError('M0 target_trade_date contains invalid values')
    observed['target_trade_date'] = parsed.dt.strftime('%Y-%m-%d')
...
def build_candidate_pipeline(numeric_features: tuple[str, ...], categorical_features: tuple[str, ...]) -> Pipeline:
    transformers: list[tuple[str, Pipeline, list[str]]] = []
    if numeric_features:
        transformers.append(('numeric', Pipeline([('imputer', SimpleImputer(strategy='median')), ('scaler', StandardScaler())]), list(numeric_features)))
    if categorical_features:
        transformers.append(('categorical', Pipeline([('imputer', SimpleImputer(strategy='most_frequent')), ('encoder', OneHotEncoder(handle_unknown='ignore'))]), list(categorical_features)))
    return Pipeline([('preprocessor', ColumnTransformer(transformers=transformers, remainder='drop')), ('classifier', LogisticRegression(**MODEL_CONSTRUCTOR))])

def calculate_metrics(y_true: np.ndarray, y_pred: np.ndarray, probabilities: np.ndarray) -> dict[str, Any]:
    ...
    return {'validation_rows': int(len(y_true)), 'accuracy': float(np.mean(y_true == y_pred)), 'balanced_accuracy': float(np.mean(list(recall.values()))), 'macro_f1': float(np.mean(list(f1.values()))), 'weighted_f1': float(sum((f1[label] * support[i] for i, label in enumerate(CLASS_ORDER))) / len(y_true)), 'multiclass_log_loss': float(-np.mean(np.log(np.clip(probabilities[np.arange(len(y_true)), target_index], np.finfo(float).eps, 1.0)))), 'B_recall': recall['B'], 'S_to_OUT_rate': float(matrix[1, 2] / support[1]) if support[1] else np.nan, 'OUT_to_S_rate': float(matrix[2, 1] / support[2]) if support[2] else np.nan, 'mean_confidence_on_incorrect_predictions': float(np.mean(confidence[incorrect])) if incorrect.any() else np.nan, 'zero_B_recall': bool(recall['B'] == 0.0), 'per_class_precision': precision, 'per_class_recall': recall, 'per_class_f1': f1, 'per_class_support': {label: int(support[i]) for i, label in enumerate(CLASS_ORDER)}, 'confusion_matrix': matrix.astype(int).tolist()}
...
def confidence_bucket(predictions: pd.DataFrame) -> dict[str, Any]:
    ...
    confidence = probabilities.max(axis=1)
    mask = (confidence >= 0.9) & (confidence <= 1.0)
    if not mask.any():
    ...
    return {'bucket_count': int(mask.sum()), 'bucket_accuracy': accuracy, 'bucket_mean_confidence': mean_confidence, 'bucket_gap': mean_confidence - accuracy, 'status': 'defined'}
...
def _evaluate_matrix(matrix_id: str, matrix: pd.DataFrame, eligible: pd.DataFrame, folds: list[tuple[np.ndarray, np.ndarray]]) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any], list[dict[str, Any]]]:
    ...
    for fold_id, (train_index, valid_index) in enumerate(folds, 1):
        ...
        for split, frame in (('train', train), ('validation', valid)):
            for feature in (*numeric, *categorical):
                ...
                null_rows.append({'matrix_id': matrix_id, 'fold_id': fold_id, 'split': split, 'feature': feature, 'row_count': len(frame), 'null_count': null_count, 'null_rate': null_count / len(frame), 'warmup_null_count': 0, 'warmup_null_rate': 0.0, 'denominator_failure_count': 0, 'denominator_failure_rate': 0.0, 'all_null': bool(frame[feature].isna().all())})
        ...
        probabilities = _align_probabilities(pipeline.predict_proba(valid.loc[:, feature_columns]), classifier.classes_)
        metrics = calculate_metrics(y_valid, predicted, probabilities)
        fold_rows.append({'matrix_id': matrix_id, 'matrix_role': MATRIX_ROLES[matrix_id], 'fold_id': fold_id, **metrics, 'acceptance_eligible': matrix_id == 'M1_REVISED_FULL'})
    ...
    aggregate = calculate_metrics(predictions['y_true'].to_numpy(str), predictions['candidate_y_pred'].to_numpy(str), predictions.loc[:, PROBABILITY_COLUMNS].to_numpy(float))
    fold_macro = np.asarray([row['macro_f1'] for row in fold_rows], dtype=float)
    aggregate.update({'fold_macro_f1_range': float(fold_macro.max() - fold_macro.min()), 'fold_macro_f1_population_standard_deviation': float(np.std(fold_macro, ddof=0)), 'minimum_fold_macro_f1': float(fold_macro.min()), 'zero_B_recall_fold_count': int(sum((bool(row['zero_B_recall']) for row in fold_rows))), 'confidence_bucket': confidence_bucket(predictions)})
...
def _m0_metrics(m0: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for fold_id, group in m0.groupby('fold_id', sort=True):
        metrics = calculate_metrics(group['y_true'].to_numpy(str), group['candidate_y_pred'].to_numpy(str), group.loc[:, PROBABILITY_COLUMNS].to_numpy(float))
    ...
    aggregate = calculate_metrics(m0['y_true'].to_numpy(str), m0['candidate_y_pred'].to_numpy(str), m0.loc[:, PROBABILITY_COLUMNS].to_numpy(float))
    fold_macro = np.asarray([row['macro_f1'] for row in rows])
    aggregate.update({'fold_macro_f1_range': float(fold_macro.max() - fold_macro.min()), 'fold_macro_f1_population_standard_deviation': float(np.std(fold_macro, ddof=0)), 'minimum_fold_macro_f1': float(fold_macro.min()), 'zero_B_recall_fold_count': int(sum((bool(row['zero_B_recall']) for row in rows))), 'confidence_bucket': confidence_bucket(m0)})
...
def _shift_rows(eligible: pd.DataFrame, m1: pd.DataFrame, folds: list[tuple[np.ndarray, np.ndarray]]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    grouped: dict[tuple[str, str], list[float]] = {}
    for fold_id, (train, valid) in enumerate(folds, 1):
        for comparison, frame, groups in (('revised_M1', m1, REVISED_GROUPS), ('mapped_M0', eligible, M0_GROUPS)):
            for group, features in groups.items():
                ...
                rows.append({'matrix_id': 'M1_REVISED_FULL' if comparison == 'revised_M1' else 'M0_FROZEN_PHASE7_2_CONTROL', 'comparison_role': comparison, 'group': group, 'fold_id': fold_id, 'feature_count': len(features), 'group_median_smd': median, 'group_max_smd': float(max(values))})
...
def evaluate_gates(m0: dict[str, Any], m1: dict[str, Any], m0_folds: pd.DataFrame, m1_folds: pd.DataFrame, shift_rows: list[dict[str, Any]], identity_status: dict[str, Any]) -> dict[str, Any]:
    success: dict[str, dict[str, Any]] = {}
    checks = {'S1': (m1['B_recall'], '>=', 0.2458904109589041, m1['B_recall'] >= 0.2458904109589041), 'S2': (m1['zero_B_recall_fold_count'], '<=', 1, m1['zero_B_recall_fold_count'] <= 1), 'S3': (m1['macro_f1'], '>=', 0.37439549155317, m1['macro_f1'] >= 0.37439549155317), 'S4': (m1['balanced_accuracy'], '>=', 0.378390605558729, m1['balanced_accuracy'] >= 0.378390605558729), 'S5': (m1['accuracy'], '>=', 0.38, m1['accuracy'] >= 0.38), 'S6': (m1['multiclass_log_loss'], '<', 1.2796950500624311, m1['multiclass_log_loss'] < 1.2796950500624311), 'S7': (m1['fold_macro_f1_range'], '<=', 0.3436553216604367, m1['fold_macro_f1_range'] <= 0.3436553216604367), 'S8': (m1['fold_macro_f1_population_standard_deviation'], '<=', 0.11025708314162237, m1['fold_macro_f1_population_standard_deviation'] <= 0.11025708314162237), 'S9': (m1['minimum_fold_macro_f1'], '>=', 0.0892156862745098, m1['minimum_fold_macro_f1'] >= 0.0892156862745098), 'S10': (m1['S_to_OUT_rate'], '<=', 0.3774193548387097, m1['S_to_OUT_rate'] <= 0.3774193548387097), 'S11': (m1['OUT_to_S_rate'], '<=', 0.49756097560975615, m1['OUT_to_S_rate'] <= 0.49756097560975615), 'S12': (m1['mean_confidence_on_incorrect_predictions'], '<=', 0.7099371, np.isfinite(m1['mean_confidence_on_incorrect_predictions']) and m1['mean_confidence_on_incorrect_predictions'] <= 0.7099371)}
    ...
    s13 = m1_bucket['bucket_count'] > 0 and m1_bucket['bucket_gap'] is not None and np.isfinite(m1_bucket['bucket_gap']) and (m1_bucket['bucket_gap'] <= 0.7 * m0_gap)
    success['S13'] = _gate('S13', m1_bucket['bucket_gap'], '<=', 0.7 * m0_gap, s13, 'inclusive 0.90-1.00 confidence bucket')
    ...
    s16 = len(ratios) == 4 and sum((value <= 0.8 for value in ratios.values())) >= 3 and all((value <= 1.1 for value in ratios.values()))
    success['S16'] = _gate('S16', ratios, 'compound', {'three_of_four': 0.8, 'none_above': 1.1}, s16, 'mapped feature shift')
    ...
    conditions = {'F1': m1['B_recall'] <= 0.1458904109589041, 'F2': m1['zero_B_recall_fold_count'] >= 2, 'F4': m1['multiclass_log_loss'] >= 1.2796950500624311, 'F5': int(np.sum(deltas >= 0.01)) < 3 or m1['fold_macro_f1_range'] > 0.3866122368679913, 'F6': not identity_status.get('zero_eligible_row_loss', False) or not identity_status.get('warmup_rates_within_limit', False), 'F7': not identity_status.get('denominator_rates_within_limit', False) or not identity_status.get('no_all_null_training_fold_feature', False), 'F8': False, 'F9': not all((success[f'S{i}']['pass'] for i in range(1, 18))), 'F10': m0_gap is None or not np.isfinite(m0_gap) or m0_gap <= 0, 'F11': not s13}
    if any((m0[key] <= 0 or not np.isfinite(m0[key]) for key in ('S_to_OUT_rate', 'OUT_to_S_rate'))):
        confusion_failure = True
    else:
        relative_improvements = [(m0[key] - m1[key]) / m0[key] for key in ('S_to_OUT_rate', 'OUT_to_S_rate')]
        confusion_failure = all((value < 0.05 for value in relative_improvements)) or any((m1[key] - m0[key] >= 0.03 for key in ('S_to_OUT_rate', 'OUT_to_S_rate')))
    ...
    return {'success_criteria': success, 'failure_criteria': failure, 'M1_final_gate_passed': bool(success['S18']['pass']), 'M0_reference_status': 'immutable_historical_control', 'M2_M5_status': 'diagnostic_only_not_eligible_for_acceptance', 'no_promotion_declaration': True}
...
def _baseline_metrics(eligible: pd.DataFrame, folds: list[tuple[np.ndarray, np.ndarray]]) -> dict[str, Any]:
    majority_frames = []
    prior_frames = []
    for _, (train, valid) in enumerate(folds, 1):
...
def _require_exact_list(payload: dict[str, Any], key: str, expected: tuple[str, ...], label: str) -> list[str]:
    value = payload.get(key)
    if not isinstance(value, list) or tuple(value) != expected:
        raise Phase74FeaturePolicyExperimentError(f'{label} {key} declaration mismatch')
...
def _validate_declared_inputs(source_manifest: dict[str, Any], dataset_manifest: dict[str, Any], feature_schema: dict[str, Any], readiness_text: str, experiment_contract: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    dataset_features = _require_exact_list(dataset_manifest, 'feature_columns', PHASE6_FEATURE_COLUMNS, 'dataset manifest')
    if feature_schema.get('schema_id') != FEATURE_SCHEMA_ID or feature_schema.get('dataset_id') != DATASET_ID:
        raise Phase74FeaturePolicyExperimentError('feature schema identity mismatch')
    schema_targets = _require_exact_list(feature_schema, 'target_columns', PHASE6_TARGET_COLUMNS, 'feature schema')
    schema_features = _require_exact_list(feature_schema, 'feature_columns', PHASE6_FEATURE_COLUMNS, 'feature schema')
    readiness_identity = _readiness_declared_identity(readiness_text)
    expected_contract_identity = {'contract_id': EXPERIMENT_CONTRACT_ID, 'contract_version': EXPERIMENT_CONTRACT_VERSION, 'design_recommendation_id': DESIGN_RECOMMENDATION_ID, 'project': PROJECT, 'lane': LANE, 'task_id': TASK_ID, 'execution_mode': EXECUTION_MODE}
    contract_identity = experiment_contract.get('experiment_identity')
    if not isinstance(contract_identity, dict) or any((contract_identity.get(key) != value for key, value in expected_contract_identity.items())):
        raise Phase74FeaturePolicyExperimentError('experiment contract identity mismatch')
    return {'source_panel_manifest': {'source_panel_id': source_panel_id, 'instruments': normalized_instruments, 'required_columns': source_columns}, 'dataset_manifest': {'dataset_id': DATASET_ID, 'feature_schema_id': FEATURE_SCHEMA_ID, 'target_source': TARGET_SOURCE, 'target_columns': dataset_targets, 'feature_columns': dataset_features}, 'feature_schema': {'schema_id': FEATURE_SCHEMA_ID, 'dataset_id': DATASET_ID, 'target_columns': schema_targets, 'feature_columns': schema_features}, 'readiness_contract': readiness_identity, 'experiment_contract': expected_contract_identity}
...
def run_experiment(request: Phase74ExperimentRequest) -> Phase74ExperimentResult:
    ...
    declared = _validate_declared_inputs(source_manifest, dataset_manifest, feature_schema, readiness_text, experiment_contract)
    ...
    declared_instruments = sorted(declared['source_panel_manifest']['instruments'])
    if observed_instruments != declared_instruments:
    ...
    for row in null_rows:
        ...
        selected = diagnostic.loc[diagnostic['feature'].eq(row['feature']) & diagnostic['row_index'].isin(selected_index)]
    ...
    identity_status = {'zero_eligible_row_loss': len(built.ordered_identities) == len(eligible), 'same_eligible_identities_as_M0_basis': built.ordered_identities.equals(eligible.loc[:, IDENTITY_COLUMNS]), 'same_320_validation_identities_as_M0': predictions['M1_REVISED_FULL'].loc[:, ['fold_id', *IDENTITY_COLUMNS, 'y_true']].equals(identities), 'no_all_null_training_fold_feature': not any((bool(row['all_null']) for row in null_rows if row['split'] == 'train')), 'warmup_rates_within_limit': bool((warmup_rates <= 0.05).all()), 'denominator_rates_within_limit': bool((denominator_rates <= 0.01).all())}
    gates = evaluate_gates(m0_aggregate, aggregate['M1_REVISED_FULL'], m0_folds, all_fold_frames[1], shift_rows, identity_status)
    baseline = _baseline_metrics(eligible, folds)
    input_identities: dict[str, dict[str, Any]] = {'source_panel': {'source_panel_id': declared['source_panel_manifest']['source_panel_id'], 'instruments': observed_instruments, 'required_columns': list(SOURCE_REQUIRED_COLUMNS)}, 'source_panel_manifest': declared['source_panel_manifest'], 'modeling_dataset': {'dataset_id': DATASET_ID, 'feature_schema_id': FEATURE_SCHEMA_ID, 'target_source': TARGET_SOURCE, 'eligible_identity_count': len(eligible)}, 'dataset_manifest': declared['dataset_manifest'], 'feature_schema': declared['feature_schema'], 'readiness_contract': declared['readiness_contract'], 'm0_validation_predictions': {'role': 'immutable_historical_control_predictions', 'row_count': len(m0), 'required_columns': list(M0_REQUIRED_COLUMNS), 'fixed_class_order': list(CLASS_ORDER), 'identity_comparison_result': True}, 'experiment_contract': declared['experiment_contract']}
    request.output_dir.mkdir(parents=True, exist_ok=True)
    paths = {name: request.output_dir / name for name in DECLARED_OUTPUT_ARTIFACTS}
    _write_json(paths['input_identity_verification.json'], {'run_id': request.run_id, 'source_git_commit_sha': request.git_commit_sha, 'inputs': {name: {'path': str(path), 'sha256': _sha256(path), 'declared_identity': input_identities[name]} for name, path in _input_paths(request).items()}, 'eligible_identity_count': len(eligible), 'validation_identity_count': len(identities), 'M0_identity_comparison_result': True, 'fold_identity_comparison_result': True, 'side_effect_declaration': {'writes_outside_output_dir': False, 'network_calls': False, 'subprocess_calls': False, 'model_serialization': False}})
    ...
    for matrix_id in MATRIX_ROLES:
        ...
        inventory[matrix_id] = {'role': MATRIX_ROLES[matrix_id], 'numeric_features': list(numeric_features), 'categorical_features': list(categorical_features), 'numeric_count': len(numeric_features), 'categorical_count': len(categorical_features), 'total_pre_encoding': len(numeric_features) + len(categorical_features), 'acceptance_eligible': matrix_id == 'M1_REVISED_FULL', 'removed_from_M1': [] if matrix_id in {'M0_FROZEN_PHASE7_2_CONTROL', 'M1_REVISED_FULL'} else [feature for feature in (*M1_NUMERIC_FEATURES, *M1_CATEGORICAL_FEATURES) if feature not in (*numeric_features, *categorical_features)], 'promotion_allowed': False}
    ...
    metric_columns = ['matrix_id', 'matrix_role', 'fold_id', 'validation_rows', 'accuracy', 'balanced_accuracy', 'macro_f1', 'weighted_f1', 'multiclass_log_loss', 'B_recall', 'S_to_OUT_rate', 'OUT_to_S_rate', 'mean_confidence_on_incorrect_predictions', 'zero_B_recall', 'acceptance_eligible']
    fold_metrics.loc[:, metric_columns].to_csv(paths['fold_metrics_by_matrix.csv'], index=False)
    _write_json(paths['aggregate_metrics_by_matrix.json'], {'fixed_class_order': list(CLASS_ORDER), 'matrices': aggregate, 'baselines': baseline})
    per_class_rows = []
    for matrix_id, metrics in aggregate.items():
        for label in CLASS_ORDER:
            per_class_rows.append({'matrix_id': matrix_id, 'class': label, 'precision': metrics['per_class_precision'][label], 'recall': metrics['per_class_recall'][label], 'f1': metrics['per_class_f1'][label], 'support': metrics['per_class_support'][label], 'status': 'defined'})
    ...
    for matrix_id in tuple(MATRIX_NUMERIC_FEATURES)[1:]:
        for metric in ('accuracy', 'balanced_accuracy', 'macro_f1', 'multiclass_log_loss'):
            ...
            ablation_rows.append({'ablation_matrix': matrix_id, 'reference_matrix': 'M1_REVISED_FULL', 'metric': metric, 'ablation_value': value, 'M1_value': reference, 'absolute_delta': value - reference, 'diagnostic_only': True, 'promotion_allowed': False})
    ...
    return Phase74ExperimentResult(output_dir=request.output_dir, artifact_names=DECLARED_OUTPUT_ARTIFACTS, eligible_row_count=len(eligible), validation_row_count=len(identities), fold_count=len(folds), final_gate_passed=bool(gates['M1_final_gate_passed']))
