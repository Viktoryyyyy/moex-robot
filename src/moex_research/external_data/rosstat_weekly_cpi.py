...
PERIOD = r'с (\d{1,2}) по (\d{1,2}) ([а-я]+) (\d{4})'
INDEX = r'(\d{1,3},\d{2})'
...
def parse(raw, *, received_at):
    """Supported scope: same-month weekly estimate, not final monthly CPI."""
    ...
    day1, day2, month, year = title.groups()
    if month not in MONTHS:
        raise ValueError('unknown observation month')
    start, end = (date(int(year), MONTHS[month], int(day)) for day in (day1, day2))
    if not 0 <= (end - start).days <= 9 or end > _utc(received_at).date():
    ...
    if not summary or summary.groups()[:4] != title.groups():
        raise ValueError('CPI summary identity or units mismatch')
    values = summary.groups()[4:]
    rows = [r for r in doc.rows if r and r[0] == 'Индекс потребительских цен (оценка)']
    if len(rows) != 1 or len(rows[0]) != 6 or rows[0][1:3] != list(values[:2]):
        raise ValueError('CPI summary and table disagree')
    headers = [r for r in doc.rows if len(r) == 4 and r[1] == 'К предыдущей дате регистрации']
    if len(headers) != 1 or headers[0][2] != f'С начала {month} {year} г.':
    ...
    return {'series_id': 'ROSSTAT_WEEKLY_CPI_ESTIMATE', 'geography': 'RU',
        ...
        'indices': dict(zip(('previous_registration', 'month_start', 'year_start'), map(str, numbers))),
        'weekly_change_percent': str(numbers[0] - Decimal('100')),
        'monthly_final': False}
