...
def _request(session: requests.Session, base_url: str, params: dict[str, object], timeout: float) -> tuple[list[str], list[list[object]]]:
    url = base_url + "/" + SOURCE_ENDPOINT.lstrip("/")
    response = session.get(url, params=params, timeout=timeout)
    response.raise_for_status()
    return _rows(response.json())


def _exact_has_data(session: requests.Session, base_url: str, secid: str, trade_date: date, timeout: float) -> bool:
...
def _month_has_data(session: requests.Session, base_url: str, secid: str, month: date, lower: date, upper: date, timeout: float) -> bool:
    ...
    current = first
    while current <= last:
        if _exact_has_data(session, base_url, secid, current, timeout):
            return True
...
def probe_coverage(secid: str, start_date: str, end_date: str, timeout: float = 30.0, apim_base_url: str | None = None) -> dict[str, object]:
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Probe canonical FORTS tradestats coverage using the exact Stage 2 APIM contract.")
