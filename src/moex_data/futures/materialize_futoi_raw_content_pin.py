...
def create_content_pin(
    *,
    ...
    for trade_date, path in partition_pairs:
        data, digest = _read_bytes_with_sha256(path)
        size = len(data)
        total_size += size
        entries.append(
            {
...
def load_and_verify_content_pin(
    ...
    if str(pin.get("raw_run_id")) != str(raw_manifest.get("run_id")):
    ...
    for item in entries:
        ...
        expected_partition_digest = _require_sha256(item.get("sha256"), "partition sha256")
        data, actual_partition_digest = _read_bytes_with_sha256(path)
        if actual_partition_digest != expected_partition_digest or int(item.get("size_bytes") or -1) != len(data):
            _fail("raw partition content changed after pin creation: " + trade_date)
        normalized_entries.append(
            {
                "trade_date": trade_date,
                "path": path,
                "sha256": expected_partition_digest,
                "size_bytes": len(data),
            }
        )
    if seen != set(by_date):
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create an immutable SHA-256 content pin for one completed canonical FUTOI raw aggregate backfill.")
