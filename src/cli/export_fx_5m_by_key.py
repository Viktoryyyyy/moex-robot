...
UA=os.getenv("MOEX_UA","moex_bot_fx_export/1.1").strip()
MSK=ZoneInfo("Europe/Moscow")
def H():
    t=os.getenv("MOEX_API_KEY","").strip()
    return {"Authorization":("Bearer "+t) if t else "","User-Agent":UA}
def getj(p,params=None):
    r=requests.get(f"{API}{p}",headers=H(),params=params,timeout=25); r.raise_for_status(); return r.json()
def to_df(b):
...
def to_5m(c:pd.DataFrame)->pd.DataFrame:
    if c.empty: return c
    ...
    return r[["end","open","high","low","close","volume"]]
def main():
    ...
    if c1.empty:
        ...
        print(pd.DataFrame(columns=["end","open","high","low","close","volume"])); sys.exit(2)
    r5=to_5m(c1)
    out=f"{secid.lower()}_5m_{day}.csv"; r5.to_csv(out,index=False)
    print(f"Ticker: {secid}"); print(f"Saved: {out} rows={len(r5)}")
    try:
