...
def load_tradestats(ticker: str, day: str) -> pd.DataFrame:
    ...
    b = j.get("data") or {}
    cols = b.get("columns", []); data = b.get("data", [])
...
def main():
    ...
    if not key:
        print("Usage: FO_KEY=<substr> [FO_DAY=YYYY-MM-DD] python export_fo_5m_by_key.py")
        sys.exit(2)
    if not day:
        day = datetime.now(TZ_MSK).date().isoformat()
    secid = resolve_fut_by_key(key, board="rfud", limit_probe_day=day)
    if not secid:
        print(f"ERROR: no futures match key='{key}'")
        sys.exit(3)
    df = load_tradestats(secid, day)
    if df.empty:
        print(f"WARN: no data for {secid} {day}")
        sys.exit(0)
