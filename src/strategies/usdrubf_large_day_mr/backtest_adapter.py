...
from src.strategies.usdrubf_large_day_mr.manifest import STRATEGY_MANIFEST
...
def _normalize_signals(signals: StrategySignalFrame) -> StrategySignalFrame:
    ...
    for row in signals:
        ...
        if float(desired_position) not in (-1.0, 0.0, 1.0):
            raise InterfaceValidationError("signal desired_position must be one of -1.0, 0.0, 1.0")
