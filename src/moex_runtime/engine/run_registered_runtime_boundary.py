...
from src.moex_runtime.state_store.file_backed_runtime_session_store import prepare_runtime_transition, recover_runtime_state, transition_journal_path, append_trade_log_row, load_runtime_state, next_trade_seq, read_last_trade_log_row, save_runtime_state
...
def run_registered_runtime_boundary(*, strategy_id: str, portfolio_id: str, environment_id: str) -> dict[str, object]:
    ...
    prior_state = recover_runtime_state(state_path, prior_state, last_trade_log_row)
    ...
    if transition.position_changed:
        ...
        trade_row = {"trade_date": trade_date, "seq": next_seq, "bar_end": latest_bar_end_iso, "action": transition.action, "prev_pos": transition.current_position, "new_pos": transition.desired_position, "price": float(close_price), "reason_code": decision.reason_code}
        prepare_runtime_transition(state_path, trade_row, transition.updated_state)
        append_trade_log_row(trade_log_path, trade_row)
    save_runtime_state(state_path, transition.updated_state)
    transition_journal_path(state_path).unlink(missing_ok=True)
