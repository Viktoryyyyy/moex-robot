...
def test_workflow_mutation_package_exists_and_is_repo_only() -> None:
    ...
    assert "selected_source_artifact_ref: null" in spec
...
def test_claim_step_is_deterministic_single_row_and_role_bound() -> None:
    spec = _read(PACKAGE_PATH)
...
def test_context_prompt_ai_and_output_steps_preserve_universal_runner_boundaries() -> None:
    ...
    assert "role_id_source: role_task.role_id" in spec
    assert "dynamic_payload_source: role_task.task_payload_json" in spec
    assert "static_context_refs_source: role_task.context_refs_json.static_context_refs" in spec
    assert "schema_refs_source: role_task.context_refs_json.schema_refs" in spec
    assert "expected_output_schema_ref_source: role_task.expected_output_schema_ref" in spec
    assert "strict_json_only: true" in spec
    assert "markdown_allowed: false" in spec
    assert "code_fences_allowed: false" in spec
    assert "external_assumptions_allowed: false" in spec
    assert "provider: Ollama" in spec
    assert "existing_ai_node_reused: true" in spec
    assert "additional_ai_nodes_allowed: false" in spec
    assert "stream: false" in spec
    assert "temperature_default: 0.1" in spec
    assert "production_live_run_in_this_phase_allowed: false" in spec
    assert "parsed_json_required: true" in spec
    assert "expected_output_schema_ref_required: true" in spec
    assert "deterministic_validation_before_persist_required: true" in spec
    assert "invalid_json_status: role_task_failed" in spec
    assert "invalid_json_blocker_code: llm_output_not_valid_json" in spec
...
def test_role_output_and_pm_l3_decision_persistence_are_untrusted_and_authority_bound() -> None:
    ...
    assert "idempotency_boundary: role_task_id_plus_output_schema_ref" in spec
...
def test_target_keeps_one_ai_node_and_one_universal_workflow_pattern() -> None:
    spec = _read(PACKAGE_PATH)
    assert "workflow_name: MOEX_ROUTE_B_WORKER_POLLER_V1_10_3" in spec
    assert "target_behavior: universal_role_runner_by_role_id" in spec
    assert "target_ai_node_count: 1" in spec
    assert "one_ai_node_per_role_allowed: false" in spec
    assert "one_workflow_per_role_allowed: false" in spec
    assert "github_executor_remains_separate_pr_only_executor: true" in spec
    assert "ai_role_direct_github_execution_allowed: false" in spec
    assert "github_execution_request_json_required: true" in spec


def test_github_execution_handoff_is_structured_request_only() -> None:
    spec = _read(PACKAGE_PATH)
    assert "github_execution_handoff_step:" in spec
    assert "applies_when_output_contains: github_execution_request_json" in spec
