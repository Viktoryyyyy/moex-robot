...
from src.moex_features.daily.usdrubf_d1_classical_indicators import INDICATOR_COLUMNS
...
def test_all_fourteen_approved_create_paths_are_present() -> None:
...
def test_feature_label_and_experiment_contracts_bind_exact_producers_and_columns() -> None:
    indicator_contract = json.loads(
        (ROOT / "contracts/features/usdrubf_d1_classical_indicators.json").read_text(encoding="utf-8")
    )
    context_contract = json.loads(
        (ROOT / "contracts/features/usdrubf_d1_ema_3_19_indicator_context.json").read_text(encoding="utf-8")
    )
    label_contract = json.loads(
        (ROOT / "contracts/labels/usdrubf_d1_ema_3_19_multi_horizon_labels.json").read_text(encoding="utf-8")
    )
    ...
    assert indicator_contract["producer_ref"].endswith(":materialize_feature_frame")
...
def test_runner_writes_exact_six_artifacts_with_input_hashes_and_quality_counts(tmp_path) -> None:
    ...
    indicator_context = pd.read_csv(output_dir / "usdrubf_d1_ema_3_19_indicator_context.csv")
    ...
    assert "ret_1d" not in indicator_context.columns
    assert "ret_3d" not in indicator_context.columns
    assert "allow_trade_h5" not in indicator_context.columns
