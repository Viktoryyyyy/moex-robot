...
def _partition(root: Path, trade_date: str, instrument_id: str = "si_futures_family", groups: tuple[str, ...] = ("FIZ", "YUR")) -> Path:
...
def _backfill_evidence(
    ...
    *,
    ...
    partitions = [_partition(root, value, instrument_id=instrument_id, groups=groups) for value in written_dates]
...
def test_first_increment_accepts_without_mutating_historical_pointer(data_root: Path) -> None:
    ...
    assert result["historical_baseline_pointer_mutated"] is False
    ...
    assert pointer["quality_status"] == "pass"
    assert pointer["cumulative_till"] == "2026-08-19"
...
def test_rejects_partition_without_balanced_group_presence(data_root: Path) -> None:
    _historical_state(data_root)
    _backfill_evidence(
        data_root,
