...
def test_futoi_seqnum_must_be_non_negative() -> None:
    with pytest.raises(
        acceptance.RawHistoryAcceptanceError,
        match="FUTOI seqnum must be non-negative",
    ):
        acceptance._validate_futoi_partition(_frame(), _expectation(), "2026-08-17")
