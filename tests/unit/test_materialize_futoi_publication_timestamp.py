...
def test_raw_source_validation_accepts_historical_archival_systime() -> None:
...
def test_reference_moment_remains_canonical_ts_and_revisions_are_preserved() -> None:
    ...
    assert cleaned["ts"].nunique() == 1
    assert cleaned["systime"].nunique() == 1
    assert set(cleaned["seqnum"].tolist()) == {195, 215}
    assert cleaned["ts"].tolist() == [
        pd.Timestamp("2021-04-06 18:30:00"),
        pd.Timestamp("2021-04-06 18:30:00"),
    ]
...
def test_publication_before_source_reference_time_fails_closed() -> None:
    frame = _normalized_rows().iloc[:1].copy()
    frame.loc[frame.index[0], "systime"] = pd.Timestamp("2021-04-06 18:29:59")
...
def test_fetch_exact_rejects_case_normalized_duplicate_columns(monkeypatch) -> None:
    frame = pd.DataFrame(
        [[6263, True, 195, "2021-04-06", "18:30:00", "Si", "FIZ", 1, 2, -1, 2, 1, "2025-06-21 16:22:46"]],
        columns=["sess_id", "SESS_ID", "seqnum", "tradedate", "tradetime", "ticker", "clgroup", "pos", "pos_long", "pos_short", "pos_long_num", "pos_short_num", "systime"],
    )
...
def test_fetch_exact_canonicalizes_unique_uppercase_source_columns(monkeypatch) -> None:
    frame = pd.DataFrame(
        [[6263, 195, "2021-04-06", "18:30:00", "Si", "FIZ", 1, 2, -1, 2, 1, "2025-06-21 16:22:46"]],
    ...
    assert list(result.columns) == ["sess_id", "seqnum", "tradedate", "tradetime", "ticker", "clgroup", "pos", "pos_long", "pos_short", "pos_long_num", "pos_short_num", "systime"]
...
def test_fetch_exact_requires_official_publication_fields(monkeypatch) -> None:
    def fake_fetch(*args, **kwargs):
        return pd.DataFrame(
            [{"sess_id": 1, "seqnum": 1, "tradedate": "2026-08-17", "tradetime": "07:05:00", "ticker": "SI", "clgroup": "FIZ", "pos": 1, "pos_long": 2, "pos_short": -1, "pos_long_num": 2, "pos_short_num": 1}]
        )
