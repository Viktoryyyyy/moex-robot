...
def test_G7_fails_when_one_enumeration_payload_key_maps_to_two_valid_timestamps(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    request, _ = _run(tmp_path, monkeypatch)
    universe = pd.read_parquet(
    ...
    first_route = universe.loc[0, "enumeration_route"]
    first_sha = universe.loc[0, "enumeration_raw_payload_sha256"]
    repeated = universe.index[
        universe["enumeration_route"].eq(first_route)
        & universe["enumeration_raw_payload_sha256"].eq(first_sha)
    ]
    assert len(repeated) > 1
    universe.loc[repeated[0], "enumeration_retrieved_at_utc"] = (
        pd.Timestamp(RETRIEVED) + pd.Timedelta(seconds=1)
    )
    candles = pd.read_parquet(
        request.output_dir / "brent_daily_candles_normalized.parquet"
