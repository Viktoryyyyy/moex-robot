    ...
    return _bytes(
        {
            ...
            "boards": {
                "columns": columns,
                "data": [[values[key] for key in columns]],
            },
        }
    )
...
def test_nonofficial_host_and_missing_metadata_column_fail() -> None:
    ...
    with pytest.raises(cny.CnyrubHistoryError, match="allowlisted") as raised:
    ...
    with pytest.raises(cny.CnyrubHistoryError, match="required identity columns") as raised:
        cny.parse_security_metadata_response(
            _bytes(metadata),
            route=cny.build_security_metadata_url(),
            retrieved_at_utc=NOW,
...
def test_pagination_advances_by_rows_until_empty_page() -> None:
        ...
        return _payload(
            [
                [
                    ...
                    1,
                    2,
                    f"2024-08-0{day} 07:00:00",
                    f"2024-08-0{day} 23:49:59",
                ]
            ]
        )
    ...
    assert [parse_qs(urlsplit(url).query)["start"] for url in calls] == [
        ["0"],
        ["1"],
        ["2"],
    ]
...
def test_semantic_failure_is_not_retried() -> None:
    ...
    with pytest.raises(ExternalDataError, match="valid UTF-8 JSON"):
        cny.fetch_cnyrub_bytes_with_retry(
            cny.build_candle_url(START, END),
            transport=transport,
...
def test_duplicate_missing_column_and_malformed_json_fail() -> None:
    ...
    with pytest.raises(cny.CnyrubHistoryError, match="missing required") as raised:
        _parse(_payload(columns=[item for item in COLS if item != "volume"]))
    assert raised.value.blocker == "official_schema_not_stable"
    with pytest.raises(cny.CnyrubHistoryError, match="valid UTF-8 JSON") as raised:
        _parse(b"not-json")
    assert raised.value.blocker == "official_schema_not_stable"
...
def test_load_security_identity_preserves_schema_blockers() -> None:
    cases = [
        (b"not-json", "official_schema_not_stable"),
        (
            _bytes({"description": {"columns": [], "data": []}}),
            "official_schema_not_stable",
        ),
    ]
    for payload, expected in cases:
        with pytest.raises(cny.CnyrubHistoryError) as raised:
            cny.load_security_identity(
                transport=lambda _url, payload=payload: payload,
                clock=lambda: NOW,
            )
        assert raised.value.blocker == expected


def test_load_security_identity_preserves_duplicate_metadata_schema_blocker() -> None:
    payload = json.loads(_metadata())
    payload["description"]["data"].append(["SECID", "SECID", "CNYRUB_TOM"])
    with pytest.raises(cny.CnyrubHistoryError) as raised:
        cny.load_security_identity(
            transport=lambda _url: _bytes(payload),
            clock=lambda: NOW,
        )
    assert raised.value.blocker == "official_schema_not_stable"


def test_load_security_identity_preserves_identity_and_provenance_classes() -> None:
    with pytest.raises(cny.CnyrubHistoryError) as identity_error:
        cny.load_security_identity(
            transport=lambda _url: _metadata(secid="CNYRUBF"),
            clock=lambda: NOW,
        )
    assert identity_error.value.blocker == "security_identity_not_reproducible"

    with pytest.raises(cny.CnyrubHistoryError) as provenance_error:
        cny.parse_security_metadata_response(
            _metadata(),
            route=cny.build_security_metadata_url().replace(
                "iss.moex.com", "example.com"
            ),
            retrieved_at_utc=NOW,
        )
    assert provenance_error.value.blocker == "provenance_not_sufficient"


def test_load_security_identity_transport_failure_is_source_classified() -> None:
    def transport(_url: str) -> bytes:
        raise ExternalDataError("external-data request failed")

    with pytest.raises(cny.CnyrubHistoryError) as raised:
        cny.load_security_identity(transport=transport, clock=lambda: NOW)
    assert raised.value.blocker == "security_identity_not_reproducible"


def test_load_security_identity_does_not_reclassify_semantic_transport_error() -> None:
    def transport(_url: str) -> bytes:
        raise cny.CnyrubHistoryError(
